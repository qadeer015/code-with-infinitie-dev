...
router.get("/progress", async (req, res) => {
    try {
        ...
        res.render("application/user/progress", { userCourses, currentCourseId: userCourses.length>0 ? userCourses[0].course_id : null, viewName: 'progress' });
    } catch (error) {
        console.error(error);
...
router.get("/achievements", async (req, res) => {
    try {
        ...
        res.render("application/user/achievements", { userCompletedCourses, viewName: 'achievements' });
    } catch (error) {
        console.error(error);
